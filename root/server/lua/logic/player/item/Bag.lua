...
function Bag:CleItem(item, log)
	local storeType = self:GetItemStoreType(item.cache.id)
	return _DelItemInBag[storeType](self, item.cache.bag_type, item, item.cache.count or 1, item.cache.bag_type, log)
end
...
function Bag:SmeltItems(itemHandles)
	local num = 0
	for _, dbid in pairs(itemHandles) do
		local item = self.itemList[dbid]
		if not item or item.cache.bag_type ~= ItemConfig.BagType.BAG_TYPE_OTHTER then
			lua_app.log_info("Bag:SmeltItems error: item dbid =", dbid, ", accountname =", self.player.cache.name)
		else
			local id = item.cache.id
			local item_config = self:GetItemConfig(id)
			if item_config.recycle~= 1 then
				lua_app.log_info("SmeltItems not exist item id =", id, ", quality =", item_config.quality)
			else
				num = num + item.cache.count * 100
				self:DelItem(dbid, item.cache.count, server.baseConfig.YuanbaoRecordType.Smelt)
			end
		end
	end
	self.player:ChangeYuanBao(num,server.baseConfig.YuanbaoRecordType.Recycle)
	server.sendReq(self.player, "sc_bag_deal_recycle_result", { state = 1, len = 1 })
end
