...
local exception = lpeg.Cmt( lpeg.Carg(1) , function ( str , pos, parser_state)
	local start = 0
	for i = 1, parser_state.line - 9 do
		start = string.find(str, "\n", start + 1)
	end
	str = string.sub(str, start + 1)
	local tmpstr
	for i = 1, 20 do
		tmpstr, str = string.match(str, "(.-)\n(.*)")
		print((9 == i and "*" or " ") .. parser_state.line - 9 + i, tmpstr)
	end
